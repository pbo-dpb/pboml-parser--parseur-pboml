...
export default class KvListSlice extends Slice {
  constructor(payload) {
    ...
    this.content =
      payload.content && !!payload.content.forEach
        ? payload.content.map(
            (el) => new KvListVariablePair(this.prototype, el),
          )
        : [];
  }
  ...
  _buildEditorInputVnodes() {
    ...
    vnodes.push(
      h(
        defineAsyncComponent(() => import("../../editors/KvSliceEditor.js")),
        { slice: this, isEditingMeta: this.state.isEditingMeta },
      ),
    );
    return vnodes;
  }
  ...
  toArray() {
    ...
    array["prototype"] = {
      key: {
        ...
        label: {
          en: this.prototype?.key.label.en,
          fr: this.prototype?.key.label.fr,
        },
      },
      value: {
        type: this.prototype?.value.type,
        label: {
          en: this.prototype?.value.label.en,
          fr: this.prototype?.value.label.fr,
        },
      },
    };
    ...
    array.content = !!this.content.forEach
      ? this.content.map((kvp) => kvp.toArray())
      : [];

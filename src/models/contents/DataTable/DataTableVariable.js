...
export default class DataTableVariable {
  ...
  constructor(payload, key) {
    ...
    this.presentation_style =
      payload?.presentation_style && payload.presentation_style !== undefined
  ...
  getTableHeaderVnode(
    ...
    if (editorStyle && this.group?.[language]) {
      groupSpan = h("span", {
        class:
          "text-gray-800 dark:text-gray-200 font-normal bg-slate-100 rounded-sm w-fit px-1",
      ...
      Array.from(Array(this.level).keys()).forEach((c) => {
        if (c < this.level - 1) {
          cellContent.push(
            h(
              "span",
              { class: "size-2 shrink-0 grow-0", ariaHidden: true },
              " ",
            ),
          );
        } else {
          ...
          cellContent.push(
            h(
              "span",
              { class: "sr-only" },
              rendererStrings[language].indented_datatable_variable.replace(
                ":level",
                this.level,
              ),
            ),

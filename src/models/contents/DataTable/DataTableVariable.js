...
export default class DataTableVariable {
    static #cellBaseClass =
        "prose dark:prose-invert border border-gray-300 dark:border-gray-700 p-1 leading-snug text-balance hyphens-auto";
    ...
    constructor(payload, key) {
        ...
        this.presentation_style =
            payload?.presentation_style &&
            payload.presentation_style !== undefined
                ? payload.presentation_style
                : defaults.presentation_style;
        this.skip_chart = payload.skip_chart ? true : false;
        this.emphasize =
            payload.emphasize !== defaults.emphasize
                ? payload.emphasize
                : defaults.emphasize;
        this.chart_type = payload.chart_type
            ? payload.chart_type
            : defaults.chart_type;
        this.level = payload.level ? payload.level : defaults.level;
        this.group = payload.group
            ? { en: payload.group?.en ?? "", fr: payload.group?.fr ?? "" }
            : null;
        this.unit = payload.unit
            ? { en: payload.unit?.en ?? "", fr: payload.unit?.fr ?? "" }
            : null;

        this.tension =
            payload.tension !== undefined ? payload.tension : defaults.tension;
    }

    static generateUniqueDataTableVariableId(label, otherVariables, groupname) {
        let key = `${groupname ? groupname : ""} ${label ? label : ""}`
            .toLowerCase()
            .replace("", "-")
            .replace(/[^_a-z0-9]+/g, "");
        ...
        while (
            !/^[a-z]/.test(key) ||
            Object.keys(DataTableEntry.defaults).includes(key) ||
            otherVariables[key] ||
            Object.values(otherVariables).some((v) => v.key === key)
        ) {
            key =
                (Math.random() + 1)
                    .toString(36)
    ...
    getTableHeaderVnode(
        ...
        if (editorStyle) {
            labelSpan = h("div", { class: `flex flex-col` }, [
                h("div", {
                    innerHTML: md.render(this.label[language]),
                    class: "pboml-prose  prose-p:leading-snug w-full prose-sm",
                }),
            ...
            labelSpan = h("span", {
                innerHTML: md.render(this.label[language]),
                class: "pboml-prose  prose-p:leading-snug w-full",
            });
        }
        ...
        if (shouldIncludeUnit && this.unit?.[language]) {
            unitSpan = h("span", {
                class: "text-gray-800 dark:text-gray-200 font-normal",
                innerHTML: md.render(this.unit[language]),
            });
        }
        ...
        if (editorStyle && this.group?.[language]) {
            groupSpan = h("span", {
                class: "text-gray-800 dark:text-gray-200 font-normal bg-slate-100 rounded-sm w-fit px-1",
                innerHTML: md.render(this.group[language]),
            });
        }

        if (this.emphasize) {
            cellClasses +=
                " bg-[rgba(254,249,195,0.8)] dark:bg-[rgba(133,77,14,0.8)]";
        } else {
            cellClasses +=
                " bg-[rgba(219,234,254,0.8)] dark:bg-[rgba(3,7,18,0.8)] lg:bg-transparent";
        }
        ...
        if (this.level > 0) {
            Array.from(Array(this.level).keys()).forEach((c) => {
                if (c < this.level - 1) {
                    ...
                    cellContent.push(
                        h(ArrowTurnDownRightIcon, {
                            class: `size-4 shrink-0 grow-0 text-gray-500 dark:text-gray-500 ${opacity}`,
                        }),
                    );
                    cellContent.push(
                        h(
                            ...
                            rendererStrings[
                                language
                            ].indented_datatable_variable.replace(
                                ":level",
                                this.level,
                            ),
                        ),
        ...
        cellContent.push(
            h("div", { class: `flex flex-col gap-.5 w-full` }, [
                groupSpan,
                labelSpan,
                unitSpan,
            ]),
        ...
        if (!editorStyle && this.type === "separator") {
            ...
            return h(
                "td",
                {
                    class: `${cellClasses}  bg-slate-100 lg:bg-slate-100 dark:bg-slate-800 dark:lg:bg-slate-800 py-0 shadow-inner`,
                    colspan,
                    ariaHidden: labelSpan ? false : true,
                },
                [
                    labelSpan
                        ? h(
                              "div",
                              {
                                  class: "flex flex-row gap-0.5 w-full py-1 font-semibold",
                              },
                              cellContent,
                          )
                        : h("div", { class: "h-1" }),
    ...
    getTableCellVnode(
        ...
        if (
            presentation_style === "inherit" &&
            owningDataTable &&
            owningDataTable.presentation_style === "prose"
        ) {
            presentation_style = "prose";
        } else if (
            presentation_style === "inherit" &&
            owningDataTable &&
            owningDataTable.presentation_style === "accounting"
        ) {
    ...
    toArray() {
        ...
        for (const [key, value] of Object.entries(defaults)) {
            if (
                deepEqual(arrayout[key], value) ||
                deepEqual(arrayout[key], { en: "", fr: "" }) ||
                deepEqual(arrayout[key], { en: "" }) ||
                deepEqual(arrayout[key], { fr: "" })
            ) {
                delete arrayout[key];
            }
        }

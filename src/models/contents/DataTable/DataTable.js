...
export default class DataTable {
  ...
  constructor(payload) {
    ...
    this.entrygroups = (Symbol.iterator in entrygroups ? entrygroups : []).map(
      (entrygroup) => {
  ...
  groupsMapForLanguage(language) {
    ...
    Object.entries(this.variables).forEach((entry) => {
      ...
      if (
        ...
        Object.keys(descriptiveVariablesThatAreAlreadyPartOfThead).includes(key)
      )
        return;
  ...
  __buildTheadNode(shouldUseGroupsPresentation, language) {
    ...
    if (this.entrygroups.length > 0) {
      ...
      let superRow = [
        h(
          "th",
          { colspan: shouldUseGroupsPresentation ? 2 : 1, scope: "col" },
          [
            h(
              "span",
              { class: "sr-only" },
      ...
      vnodes.push(
        h(
          "div",
          {
            "aria-hidden": true,
            class:
              "font-thin text-gray-800 dark:text-gray-200 border-l-2 border-gray-200 dark:border-gray-700  pl-2",
          },
          `${globalUnit}`,
        ),
      );

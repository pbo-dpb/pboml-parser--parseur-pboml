...
export default class DataTable {
    static defaults = { presentation_style: "prose", entrygroups: [] };
    ...
    constructor(payload) {
        ...
        this.content = (Symbol.iterator in content ? content : []).map(
            (entry) => {
        ...
        this.entrygroups = (
            Symbol.iterator in entrygroups ? entrygroups : []
        ).map((entrygroup) => {
        ...
        this.presentation_style =
            ...
            payload.presentation_style !== undefined
                ? payload.presentation_style
                : DataTable.defaults.presentation_style;
    ...
    groupsMapForLanguage(language) {
        ...
        const descriptiveVariablesThatAreAlreadyPartOfThead =
            this.__tHeadVariables(language);

        Object.entries(this.variables).forEach((entry) => {
            const [key, variable] = entry;

            if (
                variable.is_descriptive &&
                Object.keys(
                    descriptiveVariablesThatAreAlreadyPartOfThead,
                ).includes(key)
            )
                return;
            ...
            if (
                groups.length === 0 ||
                groups[groups.length - 1].key !== groupName
            ) {
    ...
    __buildTableRowColumnsNodes(
        shouldUseGroupsPresentation,
        key,
        variable,
        language,
        inThead,
        ...
        let columns = [];
        let headerCol = variable.getTableHeaderVnode(
            "row",
            language,
        ...
        if (isLg)
            headerCol.props["width"] =
                `${100 * (1 / (shouldUseGroupsPresentation ? 6 : 3))}%`;
        else headerCol.props["class"] += " w-32";
        columns.push(headerCol);

        if (variable.type === "separator") return columns;

        this.content.forEach((content) => {
            let cell = variable.getTableCellVnode(
                content[key],
                inThead ? "col" : false,
                language,
                content.emphasize,
            ...
            if (isLg)
                cell.props["width"] =
                    `${(100 * (2 / 3)) / this.bodyRowsCount}%`;
            else cell.props["class"] = cell.props["class"] + " w-24";
            columns.push(cell);
        });
        return columns;
    }
    ...
    __buildTheadNode(shouldUseGroupsPresentation, language) {
        ...
        if (this.entrygroups.length > 0) {
            ...
            let superRow = [
                h(
                    "th",
                    {
                        colspan: shouldUseGroupsPresentation ? 2 : 1,
                        scope: "col",
                    },
                    [
                        h(
                            "span",
                            { class: "sr-only" },
                            rendererStrings[language].empty_cell_label,
                        ),
                    ],
                ),
                ...this.entrygroups.map((entrygroup, index) => {
                    ...
                    if (entrygroup.label[language] && index % 2) {
                        classes =
                            "border-b-4 border-b-slate-400 dark:border-b-slate-600";
                    } else if (entrygroup.label[language] && !(index % 2)) {
                        classes =
                            "border-b-4 border-b-neutral-300 dark:border-b-neutral-700";
                    }

                    return h(
                        "th",
                        {
                            colspan: entrygroup.span,
                            class: `text-slate-700  dark:text-slate-300 ${classes}`,
                            scope: "col",
                        },
                        [
                            entrygroup.label[language]
                                ? entrygroup.label[language]
                                : [
                                      h(
                                          "span",
                                          { class: "sr-only" },
                                          rendererStrings[language]
                                              .empty_cell_label,
                                      ),
                                  ],
                        ],
        ...
        for (const [key, variable] of Object.entries(tHeadVariables)) {
            let columns = this.__buildTableRowColumnsNodes(
                shouldUseGroupsPresentation,
                key,
                variable,
                language,
                ...
                h("tr", {}, [
                    shouldUseGroupsPresentation
                        ? h(
                              "td",
                              {
                                  class: `text-center font-semibold text-sm text-slate-700 dark:text-slate-300 ${isLg ? "" : " w-32"}`,
                              },
                              cellGroupName
                                  ? cellGroupName
                                  : [
        ...
        groups.forEach((group) => {
            ...
            Object.entries(variables).forEach((varEntry) => {
                ...
                if (shouldUseGroupsPresentation && isFirst) {
                    let cellClasses =
                        (groupName
                            ? `${DataTableVariable.getCellBaseClass()} pboml-prose`
                            : "") + (isLg ? "" : " w-32");
                    ...
                    groupCell = h(
                        ...
                        {
                            scope: "row",
                            rowspan: Object.values(variables).length,
                            class: `bg-transparent ${cellClasses}`,
                        },
                        groupName
                            ? groupName
                            : [
        ...
        if (globalUnit)
            vnodes.push(
                h(
                    ...
                    {
                        "aria-hidden": true,
                        class: "font-thin text-gray-800 dark:text-gray-200 border-l-2 border-gray-200 dark:border-gray-700  pl-2",
                    },
                    `${globalUnit}`,
                ),
            );
        ...
        vnodes.push(
            h("div", { class: "overflow-x-auto" }, [
                h(
                    "table",
                    {
                        class: `min-w-full lg:w-full table-fixed border-collapse break-inside-avoid lg:table print:table print:text-sm`,
                    },
    ...
    toArray() {
        let array = { presentation_style: this.presentation_style };
        ...
        if (this.entrygroups.length > 0) {
            array.entrygroups = this.entrygroups.map((entrygroup) =>
                entrygroup.toArray(),
            );
        } else {

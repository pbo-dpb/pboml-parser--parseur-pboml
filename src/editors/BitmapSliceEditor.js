...
import BitmapSlice, {
  ...
  BITMAP_FORMATS,
  BITMAP_DENSITIES,
} from "../models/contents/BitmapSlice";
  ...
  setup(props, { emit, expose }) {
    ...
    const pasteFromClipboard = async () => {
      try {
        ...
        ["en", "fr"].forEach((language) => {
          if (!props.slice.content[language])
            props.slice.content[language] = newBitmapSlice.content[language];

          Object.keys(BITMAP_RESOLUTIONS).map((rs) =>
            BITMAP_DENSITIES.map((ds) =>
              BITMAP_FORMATS.map((ft) => {
                const keyForThumbnail = `${rs}_${ds}_${ft}`;
                if (
                  !props.slice.thumbnails[language]?.[keyForThumbnail] &&
                  newBitmapSlice.thumbnails[language]?.[keyForThumbnail]
                ) {
    ...
    return () =>
      h("div", { class: "flex flex-col gap-4" }, [
        h(BilingualInput, {
          label:
            strings[document.documentElement.lang].bitmap_slice_source_label,
          class: "w-full",
        ...
        h("div", { class: "grid grid-cols-2 gap-4" }, [
          h("figure", { class: "shadow-sm flex justify-center items-center" }, [
            props.slice.content.en
              ? h("img", { src: props.slice.content.en, class: "max-h-64" })
              : h("span", "❌"),
          ]),
          h("figure", { class: "shadow-sm flex justify-center items-center" }, [
            props.slice.content.fr
              ? h("img", { src: props.slice.content.fr, class: "max-h-64" })
              : h("span", "❌"),
          ]),
        ]),
          ...
          () => [
            ...
            h(
              ...
              strings[document.documentElement.lang]
                .bitmap_slice_fill_from_clipboard,
            ),
          ],
        ),
        h(
          Details,
          {
            label:
              strings[document.documentElement.lang]
                .bitmap_slice_thumbnails_details_label,
          },
          {
            default: () =>
                      ...
                      BITMAP_DENSITIES.map((ds) => {
                        return BITMAP_FORMATS.map((ft) => {
                          ...
                          return h(SingleInput, {
                            modelValue:
                              props.slice.thumbnails[lg]?.[keyForThumbnail],

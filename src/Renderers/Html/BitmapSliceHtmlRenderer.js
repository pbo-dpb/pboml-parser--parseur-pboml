...
export default class BitmapSliceHtmlRenderer extends SliceHtmlRenderer {
    buildReadonlySourceNodeForResolution(language, resolution, format) {
        const attributes = { type: `image/${format}` };

        let srcSet = [];
        BITMAP_DENSITIES.forEach((pixelDensity) => {
            let url =
                this.slice.thumbnails[language][
                    `${resolution}_${pixelDensity}_${format}`
                ];
    ...
    buildReadonlyImgNode(language) {
        return h("img", {
            src: this.slice.content[language],
            alt: this.slice.label[language],
            fetchpriority: "low",
            loading: "lazy",
        });
    }
    ...
    renderReadonlyVnode(language) {
        ...
        if (!this.slice.thumbnails[language]) {
            ...
            bitmapNode = h("figure", { class: "flex justify-center" }, [
            ...
            bitmapNode = h("picture", {}, [
                ...Object.keys(BITMAP_RESOLUTIONS).map((rs) => {
                    return BITMAP_FORMATS.map((ft) =>
                        this.buildReadonlySourceNodeForResolution(
                            language,
                            rs,
                            ft,
                        ),
        ...
        return h(
            "a",
            {
                href: this.slice.content[language],
                target: "_blank",

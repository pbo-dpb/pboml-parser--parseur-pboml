...
export default class SliceHtmlRenderer {
  ...
  _renderLabelTitleVnode(language, force = false) {
    if ((!this.slice.display_label && !force) || !this.slice.label?.[language])
      return null;
    ...
    return h(
      labelNodeType,
      { class: labelNodeClasses.join(" "), id: `${this.slice.anchor}-label` },
  ...
  renderMetaVnodes(label, content, collapsible = false) {
    ...
    return [
      h(
        ...
        {
          class:
            "flex flex-col grid-cols-3 gap-1 border-l-2 border-gray-200 dark:border-gray-700 pl-2 ",
        },
        [

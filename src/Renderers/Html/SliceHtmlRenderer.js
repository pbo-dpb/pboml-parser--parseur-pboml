...
export default class SliceHtmlRenderer {
    ...
    getLabelStringsSpanVnodesForLanguage(language) {
        ...
        return labelStrings.map((str, index) =>
            h(
                "span",
                {
                    class:
                        'not-last:after:content-["–"] after:mx-1 after:text-gray-500' +
                        (labelStrings.length > 1 ? ` first:font-normal` : ""),
                },
                str,
            ),
        );
    ...
    _renderLabelTitleVnode(language, force = false) {
        if (
            (!this.slice.display_label && !force) ||
            !this.slice.label?.[language]
        )
            return null;

        let labelNodeType = "h2";
        let labelNodeClasses = [
            "font-thin",
            "break-after-avoid",
            "text-balance",
        ];
        ...
        return h(
            labelNodeType,
            {
                class: labelNodeClasses.join(" "),
                id: `${this.slice.anchor}-label`,
            },
    ...
    __renderMetaContentArrayVnodes(contentArray, language) {
        ...
        return h(
            "div",
            { class: classes },
            contentArray.map((src) =>
                h("div", { innerHTML: md.render(src[language]) }),
            ),
        );
    }
    ...
    renderMetaVnodes(label, content, collapsible = false) {
        if (collapsible) {
            return [h(Details, { label: label }, { default: () => content })];
        }
        ...
        return [
            h(
                "dl",
                {
                    class: "flex flex-col grid-cols-3 gap-1 border-l-2 border-gray-200 dark:border-gray-700 pl-2 ",
                },
                [
                    h("dt", {
    ...
    renderSourcesVnodes(language) {
        ...
        return this.renderMetaVnodes(
            this.slice.sources.length > 1 ? "Sources" : "Source",
            this.__renderMetaContentArrayVnodes(this.slice.sources, language),
        );
    }
    ...
    renderNotesVnodes(language) {
        ...
        return this.renderMetaVnodes(
            this.slice.notes.length > 1 ? "Notes" : "Note",
            this.__renderMetaContentArrayVnodes(this.slice.notes, language),
        );
    }
    ...
    renderAltsVnodes(language) {
        ...
        return this.renderMetaVnodes(
            rendererStrings[language].alts_label,
            this.__renderMetaContentArrayVnodes(this.slice.alts, language),
            true,
        );
    }
    ...
    _renderMetaVnodes(language) {
        ...
        return metasVnodes.filter((v) => v).length
            ? h("div", { class: "flex flex-col gap-2" }, metasVnodes)
            : [];
    }
    ...
    getSliceWrapperVnode(language) {
        let props = {
            class:
                "flex flex-col gap-4 print:mt-4 @container/slice" +
                (this.slice.print_only ? " hidden print:flex" : " flex"),
        ...
        if (this.slice.presentation === "figure") {
            props = {
                ...props,
                "aria-labelledby": this.slice.label?.[language]
                    ? `${this.slice.anchor}-label`
                    : rendererStrings[language].figure_label,
                class:
                    props.class +
                    " bg-linear-to-tr from-transparent to-zinc-100 dark:to-zinc-800 rounded-tr-3xl p-4 break-inside-avoid-page pb__figure",
            };
            elType = "figure";
            ...
            props = {
                ...
                "aria-labelledby": this.slice.label?.[language]
                    ? `${this.slice.anchor}-label`
                    : rendererStrings[language].aside_label,
                class:
                    props.class +
                    " bg-linear-to-tr from-sky-50 to-sky-100 dark:from-transparent dark:to-sky-900 rounded-tr-3xl p-4 break-inside-avoid-page  pb__aside",
            };
            elType = "aside";
        }
    ...
    renderEditingVnode(language = document.documentElement.lang) {
        return h(
            ...
            { slice: this.slice, language: language, id: this.slice.anchor },
        );
    }
}

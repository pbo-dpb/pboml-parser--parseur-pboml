...
export default {
  ...
  computed: {
    ...
    scales() {
      let [descriptiveVariableKey, descriptiveVariable] =
        this.datatable.descriptiveVariableKeyPair;
      ...
      if (descriptiveVariable.type === "markdown") {
        ...
        let labels = this.datatable.content
          .map((ct) => ct[descriptiveVariableKey])
          .sort((a, b) => a - b);
    ...
    datasets() {
      ...
      if (!descriptiveVariableKey)
        throw new Error(
          "A descriptive variable must be set with `is_descriptive`=true to label the x axis.",
        );
      ...
      Object.entries(this.datatable.variables).forEach((entry) => {
        ...
        if (!variable.skip_chart && !variable.is_descriptive) {
          // TODO Document this

          const variableColor = variable.color
            ? variable.color
            : this.colorForIndex(counter, variable.emphasize);

          const varEntries = this.datatable.content
            .map((entry) => {
              if (entry.skip_chart) return null; // TODO Document this
              let valForVar = entry[currentVariableKey];
              let xLabel = entry[descriptiveVariableKey]?.[this.language]
                ? entry[descriptiveVariableKey]?.[this.language]
                : entry[descriptiveVariableKey];
              return {
                x: xLabel,
                y: valForVar?.[this.language]
                  ? valForVar[this.language]
                  : valForVar,
                backgroundColor: entry.emphasize
                  ? this.emphasizeColor(new Color(variableColor)).hex()
                  : variableColor,
                borderColor:
                  variable.chart_type !== "line" && entry.emphasize
                    ? "#a855f7"
                    : variableColor,
          ...
          let dataset = {
            ...
            cubicInterpolationMode:
              variable.tension === "monotone" ? "monotone" : "default",
            tension:
              variable.tension && variable.tension !== "monotone"
                ? variable.tension
                : null,
            ...
            borderWidth:
              variable.chart_type === "line"
                ? 3
                : varEntries.map((v) => v.borderWidth),
            pointRadius: varEntries.map((v) => v.pointRadius),
            data: varEntries,
            // Make sure we draw lines and lone points on top of bars
            order:
              counter -
              (variable.chart_type === "line" ? 10 : 0) -
              (variable.chart_type === "scatter" ? 20 : 0),
          };

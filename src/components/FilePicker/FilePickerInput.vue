<template>
    <div class="flex flex-col gap-2" :class="{ 'bg-red-100': hasError }">
        <input
            type="file"
            if="pboml-file-picker"
            @change="handleFileInputChange"
            accept=".yaml,.pdf"
        />
        <label
            v-if="hasError"
            for="pboml-file-picker"
        ...
        methods: {
            unpackPbomlDocument(pboml) {
                ...
                try {
                    ...
                    this.hasError =
                        "Unable to open file: invalid yaml // Impossible d'ouvrir le fichier : yaml non conforme";
                    return;
                }
                ...
                try {
                    ...
                    this.hasError =
                        "Unable to open file: invalid PBOML // Impossible d'ouvrir le fichier : PBOML non conforme";
                    return;
                }
                    ...
                    (arrayBuffer) => {
                        const pdfDoc = PDFDocument.load(arrayBuffer, {
                            ...
                            (pdfDoc) => {
                                ...
                                if (!subjectContent)
                                    this.hasError =
                                        "The PDF does not contain PBOML content // Le PDF ne contient pas de contenu PBOML";
                                this.unpackPbomlDocument(subjectContent);
                            },
                            () => {
                                this.hasError =
                                    "Unable to open PDF // Impossible d'ouvrir le PDF";
                            },
                        );
                    },
                    () => {
                        this.hasError =
                            "Unable to open pdf file // Impossible d'ouvrir le fichier pdf";
                    },
                );
            },
            readPbomlFile(pbomlFile) {
                new Promise((resolve, reject) => {
                    ...
                    () => {
                        this.hasError =
                            "Unable to open yaml file // Impossible d'ouvrir le fichier yaml";
                    },
                );
            },

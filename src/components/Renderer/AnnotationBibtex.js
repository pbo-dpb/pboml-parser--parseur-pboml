...
export default {
    ...
    data() {
        return { _citation: null };
    },
    render() {
        if (this._citation === null) {
            Cite.async(this.annotation.content[this.language]).then(
                (reference) => {
        ...
        return this._citation
            ? h("div", { innerHTML: this._citation })
            : h("div", {
                  class: "font-mono text-red-500",
                  innerHTML: this.annotation.content[this.language]
                      ? this.annotation.content[this.language]

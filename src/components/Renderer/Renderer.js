...
const sliceHtmlRenderers = import.meta.glob("../../Renderers/Html/*.js", {
  eager: true,
});
...
export default {
  ...
  methods: {
    _buildHeaderVnodes(language) {
      ...
      return [
        h("header", { class: "flex flex-col gap-1" }, [
          this.pbomlDocument.type?.[language]
            ? h("div", {
                class: "text-xl",
                innerHTML: this.pbomlDocument.type[language],
              })
            : null,
          this.pbomlDocument.title?.[language]
            ? h("h1", {
                class: "font-thin text-4xl text-balance",
                innerHTML: this.pbomlDocument.title[language],
              })
            : null,
          h("div", {
            class: "text-sm text-gray-800 dark:text-gray-200",
            innerHTML: this.pbomlDocument.localizedReleaseDate?.[language],
          }),
        ]),
      ];
    },
    ...
    _buildFooterVnodes(language) {
      ...
      if (this.standalone || !this.pbomlDocument.copyright?.[language])
        return [];
      ...
      return [
        h(
          ...
          {
            class:
              "flex flex-row gap-2 text-xs text-gray-800 dark:text-gray-200 justify-center items-center print:mt-8",
          },
          [
            this.pbomlDocument.copyright?.[language]
              ? h("div", {
                  class: "",
                  innerHTML: this.pbomlDocument.copyright[language],
                })
              : null,
            this.pbomlDocument.copyright?.[language] && this.pbomlDocument.id
              ? h("div", {
                  role: "separator",
                  ariaHidden: true,
                  innerHTML: "•",
              ...
              : null,
            this.pbomlDocument.id
              ? h("div", { class: "", innerHTML: this.pbomlDocument.id })
              : null,
          ],
        ),
      ];
    ...
    getSliceRenderer(slice) {
      const rendererObjectName =
        slice.constructor.rendererObjectForSliceRendererType("html");
      let htmlRenderer = new sliceHtmlRenderers[
        `../../Renderers/Html/${rendererObjectName}.js`
      ].default(slice);
      return htmlRenderer;
    },
    ...
    renderSliceAsVnode(slice, language) {
      ...
      return sliceRenderer && sliceRenderer.renderAsVnode
        ? sliceRenderer.renderAsVnode(language)
        : null;
    },
  },
    ...
    return h("div", { class: "flex flex-col gap-8 print:block", ref: "main" }, [
      ...
      ...slices.map((slice) => {
        if (Array.isArray(slice)) {
          let wrapperVnode = this.getSliceRenderer(
            slice[0],
          ).getSliceWrapperVnode(language);
          wrapperVnode.children = slice.map((s, i) =>
            this.getSliceRenderer(s).buildVnodes(language, i > 0),
          );
          return wrapperVnode;
        } else {
  ...
  mounted() {
    ...
    this.pbomlDocument.slices.forEach((slice) => {
      this.$nextTick(() =>
        this.intersectionManager.startObservingForSlice(this.$el, slice),
      );
    });
  },

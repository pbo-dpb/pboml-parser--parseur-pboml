...
export default {
    props: {
        arraytable: { type: ArrayTable, required: true },
        ...
        return h("table", { class: `table-fixed w-full` }, [
            h("thead", {}, [
                h(
                    "tr",
                    {},
                    this.headers.map((cellContent) =>
                        h("th", { class: cellBaseClass }, cellContent),
                    ),
                ),
            ]),
            h("tbody", {}, [
                this.body.map((row) =>
                    h(
                        ...
                        {},
                        row.map((celldata) =>
                            this.formatedCellForData(celldata),
                        ),
                    ),
                ),
            ]),
            ...
            if (celldata === null) {
                celldata = [
                    h(
                        "span",
                        { class: "sr-only" },
                        RendererStrings[this.language].empty_cell_label,
                    ),
                ];
                ...
                celldata = new Intl.NumberFormat(
                    this.language,
                    formatterOptions,
                ).format(celldata);
            }

<template>
    <div class="flex flex-col gap-4 rounded-sm border border-blue-800 p-4">
        <div class="grid grid-cols-2 gap-2">
            <StructureImporterDocxPicker
                @pick="handleFilePicked"
                language="en"></StructureImporterDocxPicker>
            <StructureImporterDocxPicker
                @pick="handleFilePicked"
                language="fr"></StructureImporterDocxPicker>
        </div>
        ...
        computed: {
            visualStructureHtml() {
                ...
                this.importedStructure.slices.forEach((slice) => {
                    ...
                    let contents = [
                        slice.content?.en
                            ? `<span class="text-gray-500">${slice.content.en}</span>`
                            : null,
                        slice.content?.fr
                            ? `<span class="text-gray-500">${slice.content.fr}</span>`
                            : null,
                    ].filter((c) => c);
                    ...
                    if (slice.state._structureImporterError) {
                        contents = [
                            ...contents,
                            `<span class="text-red-500">${slice.state._structureImporterError}</span>`,
                        ];
                    }
                ...
                this.importedStructure.annotations.forEach((annotation) => {
                    ...
                    let contents = [
                        annotation.content?.en
                            ? `<span class="text-gray-500te">${annotation.content.en.substring(0, 50) + "…"}</span>`
                            : null,
                        annotation.content?.en
                            ? `<span class="text-gray-500">${annotation.content.en.substring(0, 50) + "…"}</span>`
                            : null,
                        annotation.content?.fr
                            ? `<span class="text-gray-500">${annotation.content.fr.substring(0, 50) + "…"}</span>`
                            : null,
                    ].filter((c) => c);
            ...
            importedStructure() {
                ...
                const slices = englishStructure.slices.map((enSlice) => {
                    ...
                    if (!frSlice || enSlice.type !== frSlice.type)
                        enSlice.state._structureImporterError =
                            "❌ type mismatch";

                    if (
                        frSlice &&
                        frSlice.type === "heading" &&
                        enSlice.type === "heading" &&
                        frSlice.level !== enSlice.level
                    )
                        enSlice.state._structureImporterError =
                ...
                counter = 0;
                const annotations = englishStructure.annotations.map(
                    (enAnnotation) => {
                        let frAnnotation =
                            frenchStructure.annotations?.[counter] ?? {};
                        counter++;

                        if (
                            enAnnotation.content?.["en"] &&
                            frAnnotation.content?.["fr"]
                        ) {
                ...
                marked.use({
                    walkTokens: (token) => {
                        ...
                        if (token.type === "heading") {
                            ...
                            if (
                                payload.content[language] === "Notes" &&
                                payload.level === 0
                            ) {
                            ...
                            payload.id = parseInt(
                                token.text.match(/^\d+\./)?.[0] || counter + 1,
                            );

                            counter++;

                            payload.content[language] = (
                                positionOfStripping !== -1
                                    ? token.text.slice(0, positionOfStripping)
                                    : token.text
                            )?.trim();

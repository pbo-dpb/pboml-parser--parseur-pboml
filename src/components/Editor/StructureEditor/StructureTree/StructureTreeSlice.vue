<template>
    <li
        ...
        @dragenter=""
        @dragleave="isBeingDraggedOver = false"
        @drop.prevent="resetDragCues"
        @dblclick="toggleEditor">
        <ArrowsUpDownIcon
            class="h-4 w-4 text-blue-200 group-hover:text-blue-800"></ArrowsUpDownIcon>
        <div class="flex w-full flex-row items-center justify-between">
            <div class="flex items-center">
                <div
                    :title="localizedSliceType"
                    class="w-12 font-semibold">
                    {{ localizedSliceTypeAbbr }}
                </div>
                <div v-if="this.slice.state._unlocked">
                    <LockOpenIcon
                        class="size-4 pr-2 text-red-500"></LockOpenIcon>
                </div>
                <div
                    v-if="descriptor"
                    :class="{ 'font-semibold': this.slice.type === 'heading' }">
                    {{ descriptor }}
                </div>
            </div>
            <button
                @click="toggleEditor"
                class="rounded-sm p-1 hover:bg-blue-100">
                <PencilIcon
                    class="size-4 text-blue-200 group-hover:text-blue-800"></PencilIcon>
                <span class="sr-only">{{ editorStrings.edit }}</span>
            </button>
        </div>
        <StructureTreeEditDialog
            v-if="showEditor"
            :slice="slice"
            @close="toggleEditor"></StructureTreeEditDialog>
    </li>
</template>
<script>
    ...
    export default {
        ...
        computed: {
            ...
            localizedSliceTypeAbbr() {
                return (
                    this.editorStrings[`slice_type_abbrev_${this.slice.type}`] +
                    (this.slice.type === "heading" ? this.slice.level + 1 : "")
                );
            },
            descriptor() {
                let descriptor = [];
                ...
                if (this.slice.referenced_as) {
                    descriptor.push(
                        this.slice.referenced_as?.[this.language] ?? null,
                    );
                }

                if (this.slice.type === "heading") {
                    descriptor.push(
                        this.slice.content?.[this.language] ?? null,
                    );
            ...
            draggedOverClasses() {
                return {
                    "border-b-4 border-b-blue-300":
                        this.index > this.draggedSliceIndex &&
                        this.isBeingDraggedOver,

...
export default {
  ...
  mounted() {
    ...
    if (
      ...
      (!annotationIds.find((a) => a.match(/\D+/)) &&
        annotationIds.every(
          (el, index) =>
            index === 0 ||
            parseInt(el) === parseInt(annotationIds[index - 1]) + 1,
        ))
    ) {
  ...
  methods: {
    pushNewAnnotation(e, index) {
      if (!this.pbomlDocument.annotations) this.pbomlDocument.annotations = [];
    ...
    return h("section", { class: "flex flex-col gap-4" }, [
      ...
      this.pbomlDocument.annotations?.map((annotation, i) => {
        return [
          ...
          i === 0
            ? h(AnnotationsEditorNewButton, {
                soft: true,
                onClick: (e) => this.pushNewAnnotation(e, 0),
              })
            : null,
          ...
          h("div", { class: "relative" }, [
            h(
              LockStateToggler,
              { class: "absolute -left-8", unlockableObject: annotation },
              () => [],
            ),

            annotation.state._unlocked
              ? null
              : h(
                  "div",
                  {
                  ...
                  [
                    h("div", { class: "-mx-16 scale-85" }, [
                      annotation.renderAsVnode("en"),
                    ]),
                    h("div", { class: "-mx-16 scale-85" }, [
                      annotation.renderAsVnode("fr"),
                    ]),
                  ],
                ),
            ...
            annotation.state._unlocked
              ? h(AnnotationEditor, {
                  ...
                  onDelete: () => {
                    this.pbomlDocument.annotations.splice(
                      this.pbomlDocument.annotations.indexOf(annotation),
                      1,
                    );
                    this.renumberSlices();
                  },
                  onMove: (direction, e) => {
                    ...
                    const el = this.pbomlDocument.annotations.splice(
                      from,
                      1,
                    )[0];
          ...
          i === this.pbomlDocument.annotations.length - 1
            ? null
            : h(AnnotationsEditorNewButton, {
                soft: true,
                onClick: (e) => this.pushNewAnnotation(e, i + 1),
              }),
        ];
      }),
      ...
      h(AnnotationsEditorNewButton, {
        soft: this.pbomlDocument.annotations.length ? true : false,
        onClick: (e) => this.pushNewAnnotation(e),
      }),
    ]);
  },
};

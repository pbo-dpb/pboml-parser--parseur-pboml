...
export default {
    ...
    data() {
        return { strings: strings[document.documentElement.lang] };
    },
    methods: {
    ...
    render() {
        return h("div", { class: "flex flex-col gap-8" }, [
            ...
            this.pbomlDocument.slices.length === 0
                ? h(
                      defineAsyncComponent(
                          () => import("./EditorSlicesEmpty.js"),
                      ),
                      { pbomlDocument: this.pbomlDocument },
                  )
                : null,

            ...(!!this.pbomlDocument.slices.forEach
                ? this.pbomlDocument.slices.map((slice, i) => {
                      let sliceEditingVnode = slice.renderEditingVnode();
                      sliceEditingVnode.props.onDeleteSlice =
                          this.handleDeleteSlice;
                      sliceEditingVnode.props.onDuplicateSlice =
                          this.handleDuplicateSlice;
                      sliceEditingVnode.props.onMoveSlice =
                          this.handleMoveSlice;

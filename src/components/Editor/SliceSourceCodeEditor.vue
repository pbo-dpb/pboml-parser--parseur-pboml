<template>
    <div class="flex flex-col gap-4">
        <textarea
            ...
            :class="{ 'border-red-500 outline-red-500': hasError }"
            :invalid="hasError"
        ></textarea>

        <div class="flex flex-row items-start gap-4">
            <Button @click="applyCode" class="w-fit" :disabled="hasError">{{
                strings.apply
            }}</Button>
...
<script setup>
    ...
    const props = defineProps({ slice: { type: Slice, required: true } });
    ...
    const hasWarning = computed(() => {
        ...
        if (!newObject.type || newObject.type !== props.slice.type) {
            warnings.push(
                strings.value.slice_source_code_warning_type_mismatch,
            );
        }

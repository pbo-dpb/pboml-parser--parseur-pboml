    ...
    <div class="flex flex-col gap-2">
        <nav class="flex flex-row justify-end gap-2">
            <Button
                :title="
                    enhancedPreviewWindow
                        ? strings.close_enhanced_preview
                        : strings.open_enhanced_preview
                "
                :aria-pressed="enhancedPreviewWindow ? true : false"
                @click="toggleEnhancedPreview"
            >
            ...
            <Button
                :disabled="disabled"
                @click="
                    shouldDisplayPreview = shouldDisplayPreview ? false : 'en'
                "
                :toggled="shouldDisplayPreview ? true : false"
                :title="strings.editor_actions_preview"
            >
        ...
        >
            <div class="flex flex-col gap-2 h-full w-full p-4">
                <nav class="flex flex-row gap-2 justify-end align-center">
                    ...
                    <Tab
                        @click="shouldDisplayPreview = 'fr'"
                        :selected="shouldDisplayPreview === 'fr'"
                    >
                        FR
                    </Tab>

                    <button
                        @click="shouldDisplayPreview = false"
                        class="ml-8 text-blue-500 hover:text-blue-800"
...
<script>
    ...
    export default {
        props: { pbomlDocument: PBOMLDocument, disabled: Boolean },
        data() {
            return {
        ...
        components: { Button, Renderer, Eye, Tab, X, Columns2 },
        ...
        methods: {
            downloadPboml() {
                ...
                element.setAttribute(
                    "href",
                    "data:text/pboml;charset=utf-8," +
                        encodeURIComponent(rawtext),
                );
                element.setAttribute(
                    "download",
                    `${this.pbomlDocument.id}.pboml.yaml`,
                );
            ...
            injectEnhancedPreviewContent() {
                ...
                const encodedPayload = btoa(
                    unescape(
                        encodeURIComponent(this.pbomlDocument.serialize()),
                    ),
                );
                const container =
                    this.enhancedPreviewWindow.document.body.querySelector(
                        "div",
                    );

                let enPbomlPreview =
                    this.enhancedPreviewWindow.document.getElementById(
                        "en-pboml-preview",
                    );
                ...
                enPbomlPreview.setAttribute(
                    "payload",
                    `data:text/yaml;base64,${encodedPayload}`,
                );
            ...
            toggleEnhancedPreview() {
                if (!this.enhancedPreviewWindow) {
                    const enhancedPreviewTitle =
                        this.strings.enhanced_preview_title;
                    ...
                    const enhancedPreviewWindow = window.open(
                        "",
                        enhancedPreviewTitle,
                        `${desiredViewport},toolbar=no,menubar=no,location=no`,
                    );

                    if (!enhancedPreviewWindow) {
                        window.alert(
                            "The enhanced preview window was blocked by the popup blocker. Please allow popups for this site.",
                        );
                        return;
                    }

                    enhancedPreviewWindow.document.body.innerHTML =
                        newHmtl.innerHTML;
                    ...
                    enhancedPreviewWindow.document.head.appendChild(
                        pbomlScriptTag,
                    );

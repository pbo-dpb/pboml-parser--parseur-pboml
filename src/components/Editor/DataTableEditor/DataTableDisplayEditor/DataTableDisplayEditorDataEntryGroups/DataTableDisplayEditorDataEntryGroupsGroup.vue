<template>
    <div class="flex flex-col gap-4 rounded-sm bg-slate-100 p-4">
        ...
        <BilingualInput
            v-model="group.label"
            :label="strings.data_table_display_editor_data_entry_group_label">
        </BilingualInput>
                ...
                <tbody>
                    <tr>
                        <td
                            ...
                            :class="{
                                'border-t-4 border-t-cyan-800 font-semibold':
                                    entry.isInRange,
...
<script setup>
    ...
    const encompasses = computed(() => {
        ...
        if (
            !datatable.variables ||
            !datatable.content ||
            datatable.variables.length === 0 ||
            datatable.content.length === 0
        ) {
            return null;
        }

        const descriptiveVariable = Object.values(datatable.variables).find(
            (v) => v.is_descriptive,
        );
        ...
        return datatable.content.map((entry, index) => {
            ...
            return {
                label:
                    entryForVariable?.[document.documentElement.lang] ??
                    entryForVariable,
                isInRange:
                    index + 1 >= groupRange.value[0] &&
                    index + 1 <= groupRange.value[1],
            };
        });
    });
</script>

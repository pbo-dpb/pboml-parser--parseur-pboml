<template>
    ...
    >
        ...
        <div class="col-span-5 p-4">
            <DataTableDisplayEditorGeneral
                v-if="selectedTab == 'general'"
                :datatable="datatable"
            />
            <DataTableDisplayEditorVariables
                v-if="selectedTab == 'variables'"
                :datatable="datatable"
            />
            <DataTableDisplayEditorData
...
<script setup>
    ...
    const props = defineProps({ datatable: DataTable });
    ...
    const availableTabs = computed(() => {
        return [
            {
                key: "general",
                label: strings.value.data_table_display_editor_general_tab,
            },
            {
                key: "variables",
                label: strings.value.data_table_display_editor_variables_tab,
            },
            {
                key: "data",
                label: strings.value.data_table_display_editor_data_tab,
            },
        ];
    });
</script>

...
export default {
    ...
    methods: {
        ...
        getKeyVariableWrapperForRowCol(
            tables,
            rowIndex,
            rowColIndex,
            otherVariables,
            ...
            if (
                this.useLineBreakForVariableGroupName &&
                label.en &&
                label.en.includes("\n")
            ) {
                let parts = label.en.split("\n");
                group = { en: parts.shift() };
            ...
            let variable = new DataTableVariable({
                ...
                group: group,
                is_descriptive:
                    (this.locationOfVars === "row" && rowColIndex === 0) ||
                    (this.locationOfVars === "col" && rowIndex === 0),
            });

            let key = DataTableVariable.generateUniqueDataTableVariableId(
                Object.values(variable.label).filter((x) => x)?.[0],
                otherVariables,
                group?.en ?? null,
            );
            variable.key = key;
        ...
        convertVariableToNumericWhenApplicable(variables, entries) {
            ...
            const englishFormattedNumberReg = new RegExp(
                "^-?[0-9,]+\\.?[0-9]*$",
                "",
            );
            const frenchFormattedNumberReg = new RegExp(
                "^-?[0-9\\s]+\\,?[0-9]*$",
                "",
            );
            variables.forEach((variable) => {
                ...
                entries.forEach((entry) => {
                    ...
                    if (
                        typeof entry[variable.key]?.en !== "undefined" &&
                        entry[variable.key]?.en !== "-" &&
                        entry[variable.key]?.en !== "" &&
                        !englishFormattedNumberReg.test(entry[variable.key]?.en)
                    )
                        entriesAreOnlyNumeric = false;
                    if (
                        typeof entry[variable.key]?.fr !== "undefined" &&
                        entry[variable.key]?.en !== "-" &&
                        entry[variable.key]?.fr !== "" &&
                        !frenchFormattedNumberReg.test(entry[variable.key]?.fr)
                    )
                        entriesAreOnlyNumeric = false;
                });
                if (entriesAreOnlyNumeric) variable.variable.type = "number";
            });

            // Transform numeric variable entries to numbers
            entries.forEach((entry) => {
                variables
                    .filter((v) => v.variable.type === "number")
                    .forEach((numVar) => {
                        ...
                        ] of Object.entries(entry[numVar.key])) {
                        ...
                        if (firstValueForEntry) {
                            ...
                            if (languageOfFirstValueForEntry === "en")
                                ...
                                cleanedUpNumber = firstValueForEntry
                                    .replaceAll(" ", "")
                                    .replaceAll(",", ".");
                            ...
                            else entry[numVar.key] = +cleanedUpNumber;
                        }
                    });
            });
    ...
    computed: {
        dataTable() {
            ...
            bones.forEach((row, rowIndex) => {
                row.forEach((rowCol, rowColIndex) => {
                    if (this.locationOfVars === "row") {
                        if (rowIndex === 0) {
                            variables.push(
                                this.getKeyVariableWrapperForRowCol(
                                    tables,
                                    rowIndex,
                                    rowColIndex,
                                    variables,
            ...
            let transformedVarAndEntries =
                this.convertVariableToNumericWhenApplicable(variables, entries);
            variables = transformedVarAndEntries.variables;
            entries = transformedVarAndEntries.entries;

            let dataTable = new DataTable();
            dataTable.variables = Object.fromEntries(
                variables.map((v) => [v.key, v.variable]),
            );
    ...
    render() {
        ...
        let flowDom = [
            ...
            h("div", { class: "flex flex-row gap-2" }, [
                h(
                    Button,
                    { onClick: () => this.$emit("import", this.dataTable) },
                    () => [
                        h(CheckCircleIcon, { class: "w-4 h-4" }, () => []),
                        strings.data_table_importer_import_button,
                    ],
                ),
            ]),
        ];

        return h("div", { class: "flex flex-col gap-4" }, [...flowDom]);
    },
};

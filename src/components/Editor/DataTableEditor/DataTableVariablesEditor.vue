<template>
    <div
        class="grid grid-cols-7 divide-x divide-gray-300 border border-gray-300"
    >
        <div class="col-span-2 flex flex-col divide-y divide-gray-300">
            <div
                role="button"
                v-for="(variable, key) in datatable.variables"
                class="p-2 text-left"
        ...
        <div class="col-span-5 p-4">
            <DataTableVariableEditor
                v-if="currentVariableKey"
                :variable-key="currentVariableKey"
                :variable="datatable.variables[currentVariableKey]"
                @delete="handleDelete(currentVariableKey)"
            />
        </div>
    </div>
</template>
<script>
    ...
    export default {
        ...
        components: {
            ...
            Button,
            PlusIcon,
            DataTableVariablesEditorVariablePreview,
        },
        methods: {
            addVariable(userVariableKey) {
                ...
                let key = DataTableVariable.generateUniqueDataTableVariableId(
                    userVariableKey,
                    this.datatable.variables,
                );

                let prefilledVariable;
                if (
                    !this.datatable.variables ||
                    !Object.keys(this.datatable.variables).length
                ) {
                    prefilledVariable = {
                        label: { en: userVariableKey, fr: userVariableKey },
                        type: "markdown",
                        is_descriptive: true,
                    };
                } else {
                ...
                this.datatable.variables[key] = new DataTableVariable(
                    prefilledVariable,
                    key,
                );
            ...
            handleNew() {
                this.addVariable(
                    window.prompt(
                        this.editorStrings
                            .data_table_variables_editor_var_new_key,
                    ),
                );
            },
        },
        mounted() {
            if (
                ...
                this.currentVariableKey = Object.keys(
                    this.datatable.variables,
                )[0];
            }
        },

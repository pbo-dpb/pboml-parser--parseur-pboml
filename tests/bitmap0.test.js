...
test("bitmap0", async () => {
    ...
    const { container } = await render(Renderer, {
        props: { pbomlDocument, language: "en" },
    });
    ...
    const webpMd = container.querySelector(
        'picture>source[type="image/webp"][media="(max-width: 768px)"]',
    );
    expect(webpMd.getAttribute("srcset")).toBe(
        `${payload[0].slices[0].thumbnails.en.md_1x_webp} 1x, ${payload[0].slices[0].thumbnails.en.md_2x_webp} 2x`,
    );
});
